...
- Domain events are handled synchronously in-process. No external broker or invented downstream bounded context is part of the case delivery.
- Physical validator/dispenser protocols and dispense-failure recovery are outside the first delivery.
...
- A `CurrencyValidator` port checks money before mutation; the case adapter is a deterministic simulator.
...
- Errors use `application/problem+json`; clients never provide authoritative price, stock, balance, or change.
